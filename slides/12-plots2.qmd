...
-   Single variable plots exercise solutions
-   Factors
-   Multivariate plots

# Exercise solutions: single variable plots
...
-   This is not the be-all end-all list; there are more options! But these are common and useful places to start.
...
-   Adding an additional categorical variable
    ...
    -   Can you break a big plot into several small plots (facets)?
        ...
        -   No time to cover facets in lecture, but it is in your extra credit
...
::: {.column width="40%"}
```{r}
# will give me the response options
...
yrbss |> 
  ...
  mutate(text_while_driving_30d = factor(text_while_driving_30d, # change the order of the bars
                                         levels = c("did not drive", "0", 
                                                    "1-2", "3-5", "6-9",
                                                    "10-19", "20-29", "30"))) |> 
  ggplot(aes(x = text_while_driving_30d, 
             fill = gender)) + # we add the fill argument here with the variable we want the fill color to represent.
  geom_bar()
```
...
-   If this interests you, I recommend [Soc 232: Visualizing Social Data](https://visualizingsociety.com/), taught by Dr. Kieran Healy
...
-   Step 5: Refine it to make it more interpretable
    ...
    -   At minimum, add labels and drop NA values if necessary

## For tomorrow

-   No mandatory exercise on multivariate plots--but there is extra credit available if you would like some practice before diving into part 4 of your project (repo on GitHub).
-   Try out part 4 of your project. Come tomorrow with questions. 
